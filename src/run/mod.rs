...
impl ToolRun {
    ...
    fn run(&self, tool_idx: ToolIndex, bench_idx: BenchIndex) -> Res<BenchRes> {
        ...
        cmd.arg(&format!("{}", self.conf.timeout.as_secs()))
            .arg(&kid_cmd[0]);
        ...
        if time >= self.conf.timeout || Some(124) == status.code() || Some(137) == status.code() {
    ...
    fn open_stderr_file_for(&self, tool: ToolIndex, bench: BenchIndex) -> Res<File> {
        let path = self.instance.err_path_of(&self.conf, tool, bench);
    ...
    fn open_stdout_file_for(&self, tool: ToolIndex, bench: BenchIndex) -> Res<File> {
...
impl BenchRun {
    ...
    pub fn run(&mut self, bench: BenchIndex) -> Res<()> {
            ...
            loop {
                ...
                'bench_run_msgs: loop {
                    match self.from_bench_runs.try_recv() {
                        // Someone's ready.
                        Ok(Ok(index)) => {
                            if let Ok(()) = self.bench_runs[index].send(bench) {
    ...
    fn register_result(
        ...
        if let Some(code) = status.code() {
            ...
            if !disagree.is_empty() {
                let code_str = if let Some(vald_conf) = self.conf.vald_conf().get(code) {
                    &vald_conf.desc
                } else {
                    "<unknown exit code>"
                };
