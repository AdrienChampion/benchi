...
impl PlotConf {
    ...
    /// Sets the flag for considering errors as timeout.
    fn set_errs_as_tmos(&mut self, b: bool) {
        self.errs_as_tmos = b
    }
    /// Sets the flag for merging all validation results.
    fn set_merge(&mut self, b: bool) {
        self.merge = b
    }
  ...
  benchi plot my_plot.svg ...\
      ",
                ...
                .takes_value(
                    true, // ).number_of_values(
                          // 1
                ).validator(|s| PlotFmt::validator(&s))
...
pub fn plot_clap<'a>(matches: &::clap::Matches<'a>) -> Option<Res<Clap>> {
    use clap::utils::*;
...
pub fn cumul_clap<'a>(conf: PlotConf, matches: &::clap::Matches<'a>) -> Clap {
...
pub fn compare_clap<'a>(conf: PlotConf, matches: &::clap::Matches<'a>) -> Clap {
    let file_1 = matches
        .value_of("FILE_1")
