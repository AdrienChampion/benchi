...
macro_rules! bail_unimpl {
    ($conf:expr, $stuff:tt) => {
        $crate::prelude::bail!(format!("{} is not implemented yet", $conf.emph($stuff)))
    };
}
...
impl ValdConf {
    ...
    pub fn add_succ(mut self, code: Validation, info: ValdCode) -> Res<Self> {
        if let Some(ValdCode { ref desc, .. }) = self.succ.insert(code, info) {
            Err(format!(
                "code `{}` is already registered as success `{}`",
                code, desc
            )
            .into())
        } else {
    ...
    pub fn dump<W: Write>(&self, w: &mut W) -> Res<()> {
        for (code, info) in &self.succ {
            write!(w, "# success: {}, {}, {}\n", code, info.alias, info.desc)
                .chain_err(|| "while dumping validator information")?
        }
        Ok(())
    }
}
...
pub trait ValdConfExt {
    ...
    fn validators_iter(&self) -> ::std::collections::hash_map::Iter<Validation, ValdCode> {
...
impl ToolConf {
    ...
    pub fn dump_info<W: Write>(&self, conf: &Arc<RunConf>, w: &mut W) -> ::std::io::Result<()> {
        use consts::dump::*;
        ...
        if !conf.vald_conf().is_empty() {
            ...
            for (code, info) in conf.validators_iter() {
                writeln!(
                    w,
                    "  {}: {}, {}, {}",
                    vald_conf_suc_key, code, info.alias, info.desc
                )?
            }
            writeln!(w, "}}")?;
        }

        writeln!(w, "{}: {}", timeout_key, conf.timeout.as_sec_str())?;
        writeln!(w, "{}", &*cmt_pref)
    }
}
...
impl Instance {
    ...
    #[inline]
    pub fn path_of_tool(&self, conf: &Arc<RunConf>, tool: ToolIndex) -> PathBuf {
    ...
    #[inline]
    pub fn err_path_of_tool(&self, conf: &Arc<RunConf>, tool: ToolIndex) -> PathBuf {
    ...
    #[inline]
    pub fn out_path_of_tool(&self, conf: &Arc<RunConf>, tool: ToolIndex) -> PathBuf {
        let mut path = self.path_of_tool(conf, tool);
        path.push("out");
        path
    }
    ...
    #[inline]
    pub fn err_path_of(&self, conf: &Arc<RunConf>, tool: ToolIndex, bench: BenchIndex) -> PathBuf {
    ...
    #[inline]
    pub fn out_path_of(&self, conf: &Arc<RunConf>, tool: ToolIndex, bench: BenchIndex) -> PathBuf {
    ...
    #[inline]
    pub fn mk_err_dir(&self, conf: &Arc<RunConf>, tool: ToolIndex) -> Res<()> {
    ...
    #[inline]
    pub fn mk_out_dir(&self, conf: &Arc<RunConf>, tool: ToolIndex) -> Res<()> {
    ...
    #[inline]
    pub fn init_data_file_and_validator(&self, conf: &Arc<RunConf>, tool: ToolIndex) -> Res<File> {
    ...
    #[inline]
    pub fn init_validator(&self, conf: &Arc<RunConf>, tool: ToolIndex) -> Res<()> {
        ...
        if let Some(path) = conf.validator_path_of(&self[tool]) {
            ...
            if let Some(s) = self[tool].validator.as_ref() {
                ...
                file.metadata()
                    .chain_err(|| format!("could chmod validator file to executable"))?
                    .permissions()
                    .set_mode(0o744)
            }
        }
