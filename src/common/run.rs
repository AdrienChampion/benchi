...
/// `CONF` in position 1.
pub fn clap_run_subcommand<'a>() -> clap_lib::Command<'a> {
    run_subcommand(Some(
        clap::Arg::new("CONF")
...
/// argument.
pub fn conf_run_subcommand<'a, 'b>() -> clap_lib::Command<'a> {
    run_subcommand(None)
}
...
fn run_subcommand<'a>(add_arg: Option<clap_lib::Arg<'a>>) -> clap_lib::Command<'a> {
    use clap::utils::*;

    let bench_file_index = if add_arg.is_some() { 2 } else { 1 };

    let run_app = clap::Command::new("run")
        .about(
    ...
    ",
        ...
        .arg(
            clap::Arg::new("timeout")
                ...
                .validator(tmo_validator)
                .default_value("1min")
                .takes_value(true), /*.number_of_values(1)*/
        )
        .arg(
            clap::Arg::new("para_benchs")
                .long("--benchs")
...
pub fn run_clap(matches: &clap::Matches) -> Option<Res<Clap>> {
    use clap::utils::*;
    ...
    Some(Ok(Clap::Run(run_conf, tools)))
}
