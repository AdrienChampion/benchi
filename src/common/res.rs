...
impl ToolRes {
    ...
    pub fn mk(
        ...
        timeout: Duration,
        file: String,
        res: HashMap<BenchIndex, BenchRes>,
        vald_conf: ValdConf,
    ) -> Self {
    ...
    fn as_cumul_data(&self) -> Vec<Duration> {
        let mut times =
            self.res
                .iter()
                .fold(Vec::with_capacity(self.res.len()), |mut vec, (_, res)| {
                    if let BenchRes::Success(time, _) = *res {
                        vec.push(time)
                    }
                    vec
...
impl RunRes {
    ...
    pub fn check_bench_index(&mut self, bench: BenchIndex, name: String) -> Res<()> {
        match self.benchs.get(&bench) {
            Some(bench_name) => {
                if name == *bench_name {
                    ...
                    let e: Res<()> =
                        Err(format!("but was previously seen as `{}`", bench_name).into());
...
impl<'a> DataFileHandler<'a> {
    ...
    pub fn file_of<'me>(&'me mut self, code: Option<Validation>) -> Res<(&'me mut File, PathBuf)> {
        match self {
            DataFileHandler::Merged { file, path } => Ok((file, path.clone())),
            ...
            DataFileHandler::Split {
                ...
                if code.map(|code| map.contains_key(&code)).unwrap_or(false) {
                    ...
                    Ok((&mut file_n_path.0, file_n_path.1.clone()))
                ...
                } else if let Some((file, path)) = unknown {
                    Ok((file, path.clone()))
                } else {
