...
prelude!(
    common::{inspect::*, plot::*, run::*},
    consts::clap::*, clap::utils::*,
);

pub use clap_lib::{Arg, ArgMatches, Command};
...
pub mod utils {
    ...
    pub fn tmo_err(got: &str) -> Error {
        clap_err(
            "argument timeout",
            format!("expected `{}`, got {}", tmo_format, got),
        )
    }
    ...
    pub fn tmo_validator(s: &str) -> Result<(), String> {
        if let Ok(_) = tmo_of_str(&s) {
            ...
            Err(format!("expected <{}>, got `{}`", tmo_format, s))
        }
    }
    ...
    /// Loads the tool file.
    pub fn load_conf<F: AsRef<Path>>(
        conf: &GConf,
        tool_file: F,
    ) -> Res<(ValdConf, Vec<ToolConf>, ::clap_lib::ArgMatches)> {
        use clap::*;

        let mut file = File::open(&tool_file).chain_err(while_opening!(conf, tool_file))?;
        let mut buff = Vec::with_capacity(217);
        let _ = file
            .read_to_end(&mut buff)
            .chain_err(while_opening!(conf, tool_file))?;

        let (options, vald_conf, tool_confs) = crate::parse::work(&GConf::default(), &buff)?;
        ...
        let option_clap = main_app().subcommand(conf_run_subcommand());
        let matches = match option_clap.try_get_matches_from(&actual_options) {
            Ok(matches) => matches,
            Err(e) => {
                println!(
...
pub struct Matches {
    primary: ArgMatches,
    secondary: Vec<ArgMatches>,
}
impl Matches {
    /// Creates a `Matches` from an `ArgMatches`.
    pub fn mk(primary: ArgMatches) -> Self {
        Matches {
            primary,
            secondary: vec![],
        }
    ...
    pub fn push(&mut self, secondary: ArgMatches) {
    ...
    pub fn primary_values_of(&self, name: &str) -> Option<::clap_lib::Values> {
...
pub fn main_app<'a>() -> Command<'a> {
    Command::new(clap_lib::crate_name!())
        .version(clap_lib::crate_version!())
        .author(clap_lib::crate_authors!())
...
pub fn gconf_of_matches(matches: &Matches) -> GConf {
    // File overwrite.
...
/// All the subcommands.
pub fn subcommands<'a>() -> Vec<Command<'a>> {
    vec![
        clap_run_subcommand(),
        plot_subcommand(),
        inspect_subcommand(),
...
fn conf_subcommand<'a>() -> Command<'a> {
    Command::new("conf")
...
pub fn conf_clap(matches: &clap::Matches) -> Option<Res<Clap>> {
...
fn clap_fails() {
    ...
    assert!(clap.clone().try_get_matches_from(args).is_err());
