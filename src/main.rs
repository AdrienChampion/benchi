...
pub mod prelude {
    ...
    pub use crate::{
        ...
        consts,
        errors::*,
        inspect, parse, plot, run,
    };

    /// Imports this crates prelude.
    macro_rules! prelude {
        ($($stuff:tt)*) => {
            pub use $crate::prelude::{*, $($stuff)*};
        };
    }
}
...
macro_rules! while_opening {
    ($conf:expr, $file:ident) => {{
        let e = ErrorKind::Msg(format!("while opening file {}", $conf.emph(&$conf.$file)));
        || e
    }};
}
...
fn load_instance(conf: &mut RunConf, tools: Vec<ToolConf>) -> Res<Instance> {
    ...
    let mut benchs = {
        let buff_read = File::open(&conf.bench_file)
            .map(|file| BufReader::new(file))
            .chain_err(while_opening!(conf, bench_file))?;
        let mut benchs = Vec::with_capacity(200);

        for maybe_line in buff_read.lines() {
            benchs.push(maybe_line?)
        }
        benchs
    };
...
fn work(conf: Arc<RunConf>, instance: Arc<Instance>) -> Res<()> {
    ...
    let mut master = run::Master::mk(conf.clone(), instance.clone())?;
