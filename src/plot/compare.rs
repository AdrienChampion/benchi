...
pub fn work(conf: &PlotConf, file_1: String, file_2: String) -> Res<Option<String>> {
    log! { conf => "  loading tool data..." }
    let mut run_res = res::RunRes::of_files(vec![file_1.clone(), file_2.clone()])?;
    ...
    let mut data_files = res::DataFileHandler::mk(conf, &run_res)?;

    let mut res_2 = if let Some(res) = run_res.tools.pop() {
        res
    } else {
        bail!("[bug] there's no data in this run result")
    };
    let mut res_1 = if let Some(res) = run_res.tools.pop() {
        res
    } else {
    ...
    let (res_1_name, res_2_name) = (res_1.tool.name.clone(), res_2.tool.name.clone());
    ...
    for (bench, bench_res_1) in res_1.res.drain() {
        if let Some(bench_res_2) = res_2.res.remove(&bench) {
            ...
            writeln!(
                ...
                "{} {} {}",
                bench_res_1
                    .map(|time, _| time, || tmo_time, || err_time)
                    .as_sec_str(),
                bench_res_2
                    .map(|time, _| time, || tmo_time, || err_time)
                    .as_sec_str(),
...
plot \\
  ...
  x notitle with lines ls 1\
    ...
    data_files.fold_data_paths(&mut file, |file, code, index, desc, path| {
        write!(
            file,
            ...
            desc.map(|desc| format!(
                "title '{} ({})'",
                desc,
                code_to_count.get(&code).map(|cnt| *cnt).unwrap_or(0)
            ))
