...
bench name and result data.
  ...
  static ref data_success_re: Regex = Regex::new(
    r#"^\s*([0-9]*)\.([0-9]*)\s*$"#
  ).unwrap() ;
  #[doc =
    "
Regex matching a timeout result. Two groups: seconds and micros of timeout.
  ...
  static ref data_timeout_re: Regex = Regex::new(
  ...
  >(
    ...
    'lines: for line in lines {
      ...
      if let Some(cap) = runtime_re.captures(& line) {
        ...
        let data = cap.get(3).ok_or::<Error>(
          "sorry, internal problem with regex `runtime_re`, \
          please notify the developer".into()
        ) ? ;
    ...
    Ok(
      ToolData {
        tool, file: data_file, res: treatment(vec)
      }
    )
  }
}
