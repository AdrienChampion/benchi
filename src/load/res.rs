...
use super::{
    serde_error, run::{ new_codes, LCode }
} ;
...
impl NewBenchRes {
    ...
    pub fn map<
        ...
        FTmo: FnOnce() -> T, FErr: FnOnce(Duration) -> T,
...
impl LRes {
    ...
    fn finalize(
        self, gconf: & GConf, run_res: & mut RunRes, file: String
    ) -> Res<ToolRes> {
        let LRes { timeout, tool, codes, data } = self ;
        ...
        for (key, res) in data {
            use std::str::FromStr ;
            let index: BenchIdx = usize::from_str(& key).map_err::<Error, _>(
                |e| format!(
            ...
            if let Some(prev) = run_res.benchs.get(& index) {
                if prev != & bench {
                    ...
                    bail!(
                        ...
                        gconf.bad( tool.ident() ), gconf.bad(& bench), 
                        blah, gconf.sad(prev)
                    )
                }
            }
        ...
        Ok(
            ToolRes {
                tool, timeout, file, benchs, codes,
                suc_count, err_count, tmo_count
            }
        )
    }
}
...
impl LNewBenchRes {
    ...
    fn into_bench_res(self) -> Res<(String, NewBenchRes)> {
        ...
        let res = if let Some(time) = time {
            ...
            let time = Duration::from_str(& time).chain_err(
                || format!(
                    "while recovering runtime for benchmarks `{}`", bench
                )
            ) ? ;
